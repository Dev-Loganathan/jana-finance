...
test.describe.serial("admin and staff journey", () => {
  ...
  test("CSV import: dry run reports problems, then valid rows are imported as drafts", async ({ page }) => {
    await signIn(page, SUPER_EMAIL, NEW_PASSWORD, totpSecret);
    await page.getByRole("link", { name: "Customers", exact: true }).click();
    await page.getByRole("button", { name: "Import" }).click();

    const csv = [
      "firstName,lastName,gender,dob,phone,district,aadhaar",
      "Importa,Ok,FEMALE,12/03/1991,9876511111,Vellore,",
      `Importb,Dup,MALE,1990-01-01,${CUSTOMER.phone},Vellore,`,
      "Importc,Bad,MALE,not-a-date,98765,Vellore,123",
    ].join("\n");
    await page
      .getByLabel("CSV file")
    ...
    await expect(page.getByText(/Same mobile number/).first()).toBeVisible();
    ...
    await page.getByRole("tab", { name: /Partially Saved/ }).click();
    await expect(page.getByRole("row", { name: /Importa/ })).toBeVisible();
    await expect(page.getByRole("row", { name: /Importb/ })).toHaveCount(0);
  });
